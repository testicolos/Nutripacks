...
export default async function MenuPage(){
  ...
  return <main className="siteShell"><SiteHeader/>
    <section className="pageHero compactHero"><div className="contentWidth"><span className="eyebrow">Live Nutripacks menu</span><h1>Meals customers can actually select.</h1><p className="lead">Every item below is loaded from the backend. Package and cycle-week eligibility is applied when a customer builds their monthly schedule.</p><div className="filterRow"><span className="filterPill active">{catalog.menu.length} active items</span><span className="filterPill">Breakfast {catalog.menu.filter(m=>m.category==='breakfast').length}</span><span className="filterPill">Main {catalog.menu.filter(m=>m.category==='main').length}</span><span className="filterPill">Snacks {catalog.menu.filter(m=>m.category==='snack').length}</span></div></div></section>
