...
export default function MealSelectionPage(){
  ...
  const [plan,setPlan]=useState<PackageItem|null>(null); const [orderId,setOrderId]=useState<string|null>(null); const [startDate,setStartDate]=useState(''); const [selected,setSelected]=useState<Record<string,string>>({});
  ...
  return <main className="siteShell"><SiteHeader/><section className="contentWidth selectionHero"><div><span className="eyebrow">Live package menu</span><h1>Choose your first week of {plan.name} meals.</h1><p className="lead">Only meals assigned to the {plan.name} package appear below. Package quantities are validated again by the backend when you save.</p></div><div className="selectionPlanSummary"><span>{plan.duration_days}-day plan</span><strong>QAR {Number(plan.price_qar).toLocaleString()}</strong><small>{qty('breakfast')} breakfast • {qty('main')} main • {qty('snack')} snack / delivery day</small></div></section>
  <section className="contentWidth"><div className="testingBanner"><strong>Testing mode</strong><span>No payment is required. A new order becomes active as soon as a complete meal schedule is saved.</span></div></section>
  <form onSubmit={save} className="contentWidth sectionBlock"><div className="selectionToolbar"><label className="field"><span>Package start date</span><input type="date" min={today} value={startDate} disabled={Boolean(orderId)} onChange={e=>{setStartDate(e.target.value);setSelected({});}}/></label><div className="selectionNote">{orderId?'Editing an existing order. Saving will replace this order’s current selections.':'Choose every required meal for the first delivery week. Your order is created only after the full schedule passes validation.'}</div></div><div className="selectionDays">{dates.map(date=><article className="selectionDay" key={date}><div className="selectionDayHead"><span>{prettyDate(date)}</span><small>{qty('breakfast')+qty('main')+qty('snack')} selections</small></div>{(['breakfast','main','snack'] as const).map(slot=>Array.from({length:qty(slot)}).map((_,index)=>{const key=`${date}:${slot}:${index}`;return <label className="mealSelect" key={key}><span>{slot==='main'&&qty(slot)>1?`Main meal ${index+1}`:slot.charAt(0).toUpperCase()+slot.slice(1)}</span><select value={selected[key]||''} onChange={e=>setSelected(prev=>({...prev,[key]:e.target.value}))} required><option value="">Choose a meal</option>{eligible(slot).map(item=><option value={item.id} key={item.id}>{item.name} • {item.calories} kcal • {Number(item.protein_g)}g protein</option>)}</select></label>}))}</article>)}</div>{error&&<div className="selectionError">{error}</div>}<div className="selectionSaveBar"><div><strong>{Object.values(selected).filter(Boolean).length}</strong><span> meals selected for this week</span></div><button className="button buttonPrimary" type="submit" disabled={saving}>{saving?'Saving selections…':orderId?'Update meal schedule':'Save meals & place test order'}</button></div></form><SiteFooter/></main>;
}
