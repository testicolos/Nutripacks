...
type Row={selection_id:string;delivery_date:string;meal_slot:string;quantity:number;meal_name:string;order_number:string;order_status:string;payment_status:string;package_name:string;customer_name:string;customer_email?:string|null;customer_phone?:string|null;delivery_address?:string|null;delivery_slot?:string|null};
type Total={delivery_date:string;meal_slot:string;meal_name:string;total_quantity:number};
type Schedule={role:string;from:string;to:string;rows:Row[];totals:Total[]};
...
export default function StaffPortal(){
  ...
  const today=new Date(); const next=new Date(); next.setDate(next.getDate()+7);
  const [from,setFrom]=useState(iso(today)); const [to,setTo]=useState(iso(next));

  async function loadSchedule(currentFrom=from,currentTo=to){
    const response=await fetch(`/api/staff/schedule?from=${currentFrom}&to=${currentTo}`,{cache:'no-store'});
    const data=await response.json(); if(!response.ok){setError(data.error||'Unable to load schedule.');return;} setSchedule(data);
  }
  async function load(){
    setLoading(true); setError('');
    const meRes=await fetch('/api/staff/me',{cache:'no-store'});
    if(!meRes.ok){router.replace('/staff/login');return;}
    const who=await meRes.json(); setMe(who);
    await loadSchedule();
    if(who.role==='admin'){const u=await fetch('/api/staff/users',{cache:'no-store'});if(u.ok)setUsers(await u.json());}
    setLoading(false);
  }
  useEffect(()=>{load();},[]);
  ...
  return <main className="staffPortal">
    <div className="staffTopbar"><div><strong>Nutripacks Operations</strong><span className={`rolePill role-${me.role}`}>{me.role}</span></div><div><span>{me.username}</span>{me.role==='admin'&&<a href="/admin">Catalog admin</a>}<button onClick={signOut}>Sign out</button></div></div>
    <div className="staffContent">
      <section className="staffHero"><div><span className="eyebrow">Live operations</span><h1>{me.role==='chef'?'Kitchen production schedule':me.role==='sales'?'Customer selections & sales view':'Operations overview'}</h1><p>{me.role==='chef'?'Meal quantities and customer selections, with customer contact details intentionally hidden.':'Review customer meal selections, payment status, delivery timing and kitchen totals.'}</p></div><form className="staffDateFilter" onSubmit={applyRange}><label>From<input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>To<input type="date" value={to} onChange={e=>setTo(e.target.value)}/></label><button className="button buttonPrimary buttonSmall">Refresh</button></form></section>
      {error&&<div className="selectionError">{error}</div>}
      <section className="staffStats"><div><span>Selected meal units</span><strong>{(schedule?.rows||[]).reduce((n,r)=>n+Number(r.quantity||0),0)}</strong></div><div><span>Delivery days</span><strong>{grouped.length}</strong></div><div><span>Customer lines</span><strong>{schedule?.rows?.length||0}</strong></div><div><span>Paid selections</span><strong>{(schedule?.rows||[]).filter(r=>r.payment_status==='paid').length}</strong></div></section>
      <section className="staffPanel"><div className="panelHeader"><div><span className="eyebrow">Chef view</span><h2>Kitchen totals by delivery day</h2></div></div>{grouped.length===0?<p>No customer meal selections exist in this date range yet.</p>:<div className="kitchenDays">{grouped.map(([date,items])=><article className="kitchenDay" key={date}><h3>{pretty(date)}</h3>{items.map(item=><div className="kitchenTotal" key={`${item.meal_slot}-${item.meal_name}`}><div><span>{item.meal_slot}</span><strong>{item.meal_name}</strong></div><b>× {item.total_quantity}</b></div>)}</article>)}</div>}</section>
      <section className="staffPanel"><div className="panelHeader"><div><span className="eyebrow">Customer schedule</span><h2>Selections & delivery details</h2></div></div><div className="staffTableWrap"><table className="table staffTable"><thead><tr><th>Date</th><th>Customer</th><th>Package</th><th>Meal</th><th>Qty</th><th>Delivery</th>{me.role!=='chef'&&<><th>Contact</th><th>Payment</th></>}</tr></thead><tbody>{(schedule?.rows||[]).map(row=><tr key={row.selection_id}><td>{pretty(row.delivery_date)}</td><td><strong>{row.customer_name}</strong><small>{row.order_number}</small></td><td>{row.package_name}</td><td><strong>{row.meal_name}</strong><small>{row.meal_slot}</small></td><td>{row.quantity}</td><td><strong>{row.delivery_slot||'—'}</strong><small>{row.delivery_address||'No address'}</small></td>{me.role!=='chef'&&<><td><strong>{row.customer_phone||'—'}</strong><small>{row.customer_email||''}</small></td><td><span className={`paymentStatus ${row.payment_status}`}>{row.payment_status}</span></td></>}</tr>)}</tbody></table></div></section>
